...
const server = http.createServer(async (req, res) => {
  ...
  const blogMatch = url.pathname.match(/^\/api\/battlelog\/(.+)$/);
  if (blogMatch && req.method === 'GET') return handleBattleLog(res, blogMatch[1]);
...
server.listen(PORT, () => {
  ...
  console.log('    GET /api/battlelog/:tag → Player battle log\n');
});
